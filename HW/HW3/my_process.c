...
struct cred *cred; //Structure needed to access the credentials of each task 
...
static int process_init(void)
{
        ...
        for_each_process( task ){  // for_each_process() MACRO for iterating through each task in the os located in linux/sched/signal.h
                cred = get_task_cred(task); // get the credentials of the task
                printk(KERN_INFO "PARENT PID: %d, UID: %d, PROCESS: %s\n",task->pid, cred->uid, task->comm); // log parent id/executable name/state
